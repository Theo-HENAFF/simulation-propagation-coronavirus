        ...
        for person in liste_pers:
            ...
            if person.health_status != "dead":
                for id_neighbour in person.liste_neighbour:
                    ...
                    elif (liste_pers[id_neighbour].health_status != "contaminated"
                          or person.health_status != "contaminated") and decision(proba_meet):
                        meeting(person, id_neighbour)
                    # Si une des 2 a des symptomes, la proba de rencontre est grandement diminué
                    elif (liste_pers[id_neighbour].health_status == "contaminated"
                          or person.health_status == "contaminated") and decision(proba_meet/malus_conta):
                        meeting(person, id_neighbour)
                    else:
...
ax.stackplot(x, stats["cont_without_s"], stats["contaminated"], stats["dead"], stats["cured"],
             colors=pal, alpha=0.9, labels=labels)
